 ...
 * 先思路，后撸码。 码道 ---- ‘先礼后兵’。
 * 
 * 2022-09-16 更新： 下面思路都为第一版的初始想法，与具体代码逻辑有实际出入
 * 
 *  > 一些黑话单词：
...
export const filterConnectionElementAll = (targetElement, filtrateElements) => {
  ...
  const recursionFindIncoming = (currentTargetElement) => {
    ...
    if (!currentTargetElement?.incoming.length) {
      leftRootElement.push(currentTargetElement);
      return;
    }
    ...
    currentTargetElement.incoming.forEach((connectionElement) => {
      ...
      if (filtrateElements && filtrateElements.length && !filtrateElements.includes(incomingElement)) {
        leftRootElement.push(currentTargetElement);
        return;
      }
  ...
  // 2. 递归寻找所有支出连接的元素
  const recursionFindOutgoing = (currentTargetElement) => {
    // 结束递归
    ...
    currentTargetElement.outgoing.forEach((connectionElement) => {
      ...
      outgoingElement && results.push(outgoingElement);
      recursionFindOutgoing(outgoingElement);
    });
  }
  // 启动第二次递归
  leftRootElement.forEach(rootElement => {
    recursionFindOutgoing(rootElement);
    rootElement && results.push(rootElement);
  });

  // 3. 如果有指定范围集合，那就再过滤一下
  if (filtrateElements && filtrateElements.length) {
    return [...new Set(results)].filter(element => filtrateElements.includes(element));
  }
...
export const insertPlaceMoveElement = (args, latestPosition, insertPlaceholderElement) => {
  ...
  if (isExistChildren) {
    ...
    const leftLowerElements = getRightUpperPointToLeftLowerAreaElements(insertPlaceholderElement, allShapeElements);
    const rightLowerElements = getLeftUpperPointToRightLowerAreaElements(insertPlaceholderElement, allShapeElements);
    const lastRightNeedMoveElements = getLastNeedMoveElements(filterConnectionElementAll(insertPlaceholderElement, rightLowerElements), allShapeElements);

    modeling.moveElements(leftLowerElements.concat(lastRightNeedMoveElements), {
      x: 0,
